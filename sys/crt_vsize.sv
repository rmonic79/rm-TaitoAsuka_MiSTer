...
module crt_vsize #(
    ...
    reg  [23:0]    ring_q;
    // ring_r: una copia di ring_q IN LOGICA, dopo il mux fra i blocchi M10K.
    // Il ring e diviso su molti blocchi e Quartus mette il mux DOPO il registro
    // di uscita della RAM: senza ring_r il cammino era uscita M10K -> mux ->
    // DSP del quadrato -> registro, in un colpo solo (misurato a -0,016 ns).
    // Serve solo al Cabinet, che ha la sua schedula anticipata di un colpo.
    reg  [23:0]    ring_r;
    initial begin rd_addr = 0; rd_addr_t = 0; rd_addr_m = 0; ring_q = 0; ring_r = 0; end
    always @(posedge clk) begin
    ...
    always @(posedge clk) begin
        if (pxl_cen) begin
            // Indirizzo A UN COLPO PRIMA della fase 0, sul fronte stesso di
            // pxl_cen: t_px qui vale esattamente cio che t_px_l riceve su
            // questo fronte, e t_slotA cambia solo sui fronti hs_rise, dove
            // t_fetch va a 0 e il pixel esce non valido (t_out_v = 0, e
            // l uscita mostra t_out solo con t_out_v alto).
            rd_addr_t <= line_base(t_slotA) + t_px;
            if (hs_in & ~hs_d) begin
                t_px    <= 0;
                t_fetch <= 0;
            end else begin
        ...
        // fasi, contando i fronti da quello di pxl_cen (E):
        //   E    rd_addr_t <= A          E+1  rd_addr_m <= A, rd_addr_t <= B
        //   E+2  ring_q <= ring[A]       E+3  ring_q <= ring[B], ring_r <= A
        //   E+4  t_sqa <= A^2, ring_r <= B          E+5  t_sqb <= B^2
        // t_sqa e t_sqb si caricano sugli STESSI fronti e con gli STESSI valori
        // di quando indirizzi e quadrati stavano alle fasi 0-1 e 3-4 su ring_q:
        // cambia solo che fra la RAM e il DSP c e un registro.
        case (p_cnt)
            5'd0: rd_addr_t <= line_base(t_slotB) + t_px_l;
            5'd3: begin
                t_sqa_r <= ring_r[23:16] * ring_r[23:16];
                t_sqa_g <= ring_r[15:8]  * ring_r[15:8];
                t_sqa_b <= ring_r[7:0]   * ring_r[7:0];
            end
            5'd4: begin
                t_sqb_r <= ring_r[23:16] * ring_r[23:16];
                t_sqb_g <= ring_r[15:8]  * ring_r[15:8];
                t_sqb_b <= ring_r[7:0]   * ring_r[7:0];
            end
            5'd5: begin
                t_bl_r <= t_mix_r[23:8];
                t_bl_g <= t_mix_g[23:8];
