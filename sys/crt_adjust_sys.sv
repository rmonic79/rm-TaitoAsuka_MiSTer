...
(
    ...
    //
    // Il read side emette la riga PRECEDENTE (~bank). Prima vb_active era vb_in
    // campionato a hs_rise e ritardato con DUE registri, e su rmSlapFight la fase
    // di vb_in non e' una sola:
    //  - V-Size a zero: vb_in e' CRT_VBL, che cambia a hcnt 0 mentre l'HSync sale
    //    a hcnt 320. La riga coi pixel di vcnt 0 comincia all'hs_rise di vcnt 269,
    //    dove VBlank vale ancora 1: campionato li' e' gia' una riga in ritardo sul
    //    DE. Con i due registri la PRIMA riga attiva (vcnt 0) veniva spenta.
    //  - V-Size PVM: vb_in e' l'o_vb di crt_vsize, generato per riga d'uscita,
    //    con un'altra fase. Una correzione tarata su una fase sola sbaglia l'altra
    //    (SeibuCup, col vb allineato, ci perdeva l'ultima riga).
    // Quindi la finestra si ricava dal CONTENUTO del linebuffer, come su SeibuCup:
    // se la riga appena scritta ha avuto pixel attivi, quella che il read rigioca
    // adesso li ha. Vero per costruzione, con qualunque fase di vb_in.
    reg line_act_wr;   // la riga in scrittura ha avuto pixel attivi
    reg line_act_rd;   // ... quella che il read side sta emettendo ora
    ...
    always @(posedge clk) begin
        if (bypass) begin
            ...
            if (pxl2_cen) begin
                ...
                // In ramo attivo si pubblica il flusso EMESSO, come fa gia' hb_out
                // con ~pass_q. vb_in_q e' il lato di scrittura, una riga avanti al
                // read: mentre si rigiocava vcnt 239 valeva gia' il VBlank di vcnt
                // 240, e a valle (vga_de_hs = ~hb & ~vb) spegneva l'ULTIMA riga.
                vb_out <= vb_active;
            end
        end
    end
