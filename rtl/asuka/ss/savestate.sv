...
module save_state_data #(parameter COUNT = 16)(
    ...
    input        read_start,
    input        write_start,
    input  [3:0] index,   // Darius: 16 slot (delta vs BW 4 slot)
    output       busy,
...
//                      header proprio + stato ~150KB << 448KB)
...
// da +8 a fine sotto-slot 3) -> il Main scrive il file con TUTTI e 4 i
// sotto-slot dentro. Load: lettura diretta del sotto-slot (i file sono gia'
// in DDR dal boot). Rimozione 16-slot: start_addr = base + index*0x200000.
