...
(
    ...
    output reg  [7:0]   chunk_select,  // reg: assegnato in always_comb (compat vsim, identico in sintesi)

    output              busy,
    // [FIX slot vuoto] 1 = l'ultimo LOAD ha trovato un header sotto-slot NON valido
    // (magic assente): sotto-slot mai scritto -> nessuno scatter fatto. Latchato
    // fino al load successivo; il restore 68k lo usa per NON far ripartire il core.
            ...
            case (state)
                    ...
                    if (~ddr.busy) begin
                        ...
                        if (ddr.rdata_ready) begin
                            ...
                            if (is_reading) begin
                                ...
                                //  - magic bit[63:62]==2'b11 (il save scrive 2'b11; un
                                //    sotto-slot mai scritto ha residui -> quasi mai 11)
                                //  - size in [1 .. length/4] (range plausibile)
                                // Se INVALIDO -> sotto-slot non scritto: NESSUNO scatter
                                // (vai a IDLE senza READ_MEM_REQ) + alza slot_empty ->
                                // il restore 68k non fara' ripartire il core.
                ...
                WRITE_HEADER: begin
                    ...
                    if (~ddr.busy) begin
                        ...
                        // [FIX slot vuoto] magic 2'b11 nei bit alti (prima 2'b00 fissi):
                        // marca il sotto-slot come SCRITTO. Il load lo valida; un
                        // sotto-slot mai scritto (residuo) non ha 2'b11 -> trattato vuoto.
